...
@Component({
  selector: 'compass-ui-rsdi-changes',
  templateUrl: './rsdi-changes.component.html',
  styleUrls: ['./rsdi-changes.component.css']
})
export class RSDIChangesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.individualInformations.forEach((element: IIndividualInformation, index: number) => {
      ...
      formGroup.get('frequency')?.setValue(element.employmentInformation?.retirementSurvivorDisabilityIncome?.frequency);
      formGroup.get('grossAmount')?.setValue(element.employmentInformation?.retirementSurvivorDisabilityIncome?.grossAmount);
  ...
  onNext() {
    ...
    if (invalidGroups === 0) {
      ...
      this.reportChangesService.navigateToNextIncomeChange('RSDI_CHANGES');
    }
  }
}
