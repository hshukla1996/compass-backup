...
import { IChangeReportState, IIndividualInformation } from '../../+state/models/change-report/change-report.model';
...
@Component({
  selector: 'compass-ui-unemployment-screen',
  templateUrl: './unemployment-screen.component.html',
  styleUrls: ['./unemployment-screen.component.css']
})
export class UnEmploymentScreenComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.individualInformations.forEach((element: IIndividualInformation, index: number) => {
      ...
      formGroup.get('frequency')?.setValue(element.employmentInformation?.unemploymentIncome?.frequency);
      formGroup.get('grossAmount')?.setValue(element.employmentInformation?.unemploymentIncome?.grossAmount);
  ...
  onNext() {
    ...
    if (invalidGroups === 0) {
      ...
      this.reportChangesService.navigateToNextIncomeChange('UNEMPLOYMENT_SCREEN');
    }
  }
}
