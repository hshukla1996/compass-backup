...
import { ApplyNowHouseholdCashAssistanceStrategy } from '../../../shared/route-strategies/apply-now/household-cash-assistance';
import { ScreenQueueUtil } from '../../../shared/services/screen_queue_util.service';

@Component({
  selector: 'compass-ui-household-cash-assistance',
  templateUrl: './household-cash-assistance.component.html',
  styleUrls: ['./household-cash-assistance.component.scss'],
  providers: [ApplyNowHouseholdCashAssistanceStrategy]
})
export class HouseholdCashAssistanceComponent implements OnInit {
  public age: any;
  ...
  householdCashAssistanceForm: FormGroup | any;
  ...
  constructor(
    ...
    private routingStratagy: ApplyNowHouseholdCashAssistanceStrategy,
  ...
  ngOnInit(): void {
    this.householdCashAssistanceForm = this.fb.group({
      id: this.fb.array([]),
    });
    ...
    if (this.applyNowState.houseHoldDetails) {
      ...
      if (this.selectedUserids.length === 0) {
        this.applyNowState.houseHoldDetails.selectedForCashAssitance.forEach((ind) => {
          if (ind) {
  ...
  get f() { return this.householdCashAssistanceForm.controls; }
  ...
  onSubmit(): boolean {
    if (this.selectedUserids.length > 0) {
      const storeHouseholdCashAssistance = this.applyNowState.houseHoldDetails;
      const updatedHouseholdCashAssistance = { selectedForCashAssitance: this.selectedUserids }
      this.service.updateHouseHoldDetails({ ...storeHouseholdCashAssistance, ...updatedHouseholdCashAssistance })
